...
class DetectLevelNode(Node):
    ...
    def __init__(self):
        ...
        self.pub_cmd_vel_state = self.create_publisher(Float64, '/control/cmd_vel_state', 1)
        ...
        self.declare_parameter('detect.level.red.hue_h', 26, descriptor=hue_h_descriptor)
        self.declare_parameter('detect.level.red.saturation_l', 17, descriptor=sat_l_descriptor)
        self.declare_parameter('detect.level.red.saturation_h', 255, descriptor=sat_h_descriptor)
        self.declare_parameter('detect.level.red.lightness_l', 69, descriptor=light_l_descriptor)
        ...
        self.sub_image_type = 'compressed'  # 'raw' or 'compressed'
        ...
        if self.sub_image_type == 'compressed':
            self.create_subscription(
                CompressedImage, '/camera/image_raw/compressed', self.get_image, 10)
    ...
    def level_crossing_order(self, order_msg):
        ...
        state = 0.0
        self.get_logger().info(f'[DEBUG] order_msg.data: {order_msg.data}')
        self.get_logger().info(f'[DEBUG] expected value: {self.StepOfLevelCrossing.pass_level.value}')
        if order_msg.data == self.StepOfLevelCrossing.pass_level.value:
            while rclpy.ok():
                ...
                if is_level_detected:
                    ...
                    self.get_logger().info(f'[publish] max_vel: {max_vel_msg.data}')
        ...
        msg= Float64()
        msg.data = state
        self.pub_cmd_vel_state.publish(msg)
        time.sleep(3.0)
