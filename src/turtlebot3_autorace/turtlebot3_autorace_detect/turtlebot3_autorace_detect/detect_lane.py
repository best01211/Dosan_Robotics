...
class DetectLane(Node):
    ...
    def __init__(self):
        ...
        self.pub_white_pixel_count = self.create_publisher(Int32, '/detect/white_pixel_count', 1)
    ...
    def maskWhiteLane(self, image):
        ...
        # 점선 연결을 위한 morphology close (끊긴 차선을 하나로 연결)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 3))
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)

        fraction_num = np.count_nonzero(mask)

        # 밝기 보정 자동 튜닝 (optional)
        ...
        # 신뢰도 계산
        how_much_short = 600 - sum(
            1 for i in range(600) if np.count_nonzero(mask[i, :]) > 0
        )
        ...
        self.pub_white_line_reliability.publish(msg_white_line_reliability)
                ...
                # 신뢰도 퍼블리시 아래에 추가:
        msg_white_pixel_count = Int32()
        msg_white_pixel_count.data = fraction_num
        self.pub_white_pixel_count.publish(msg_white_pixel_count)
