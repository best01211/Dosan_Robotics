...
class ImageProjection(Node):
    ...
    def cbImageProjection(self, msg_img):
        ...
        #cv_image_original = self.automatic_gamma_corection(cv_image_original)

        cv_image_original = cv2.convertScaleAbs(cv_image_original, alpha=1.0, beta=-18) # 수동
        
        # cv_image_original = self.auto_exposure(cv_image_original) # 자동
        # cv_image_original = self.white_balance(cv_image_original)
    ...
    #    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(32, 32)) #16, 16  #32, 32
